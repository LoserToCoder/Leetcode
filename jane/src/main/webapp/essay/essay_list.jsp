		...
		<div class="container index">
			<div class="row">
				...
				<div class="col-xs-16 main">
					...
					<div></div>
					<c:if test="${beans.pageTotal>=1}">
					<div class="meta" style="margin-left: 5px;margin-bottom: 20px;font-size: 15px;font-weight: 400" id="meta_info">
						<span><em style="color: red">创作共享平台</em>为您找到相关结果共<em style="color: red;">${beans.resultTotal}</em>条数</span>
						&nbsp&nbsp<span>耗时<em style="color: red;">${beans.took}</em>s</span>
					</div>
					<div class="split-line"></div>
					<div></div>
					</c:if>
					<c:if test="${beans.pageTotal<1}">
						<div class="meta" style="margin-left: 5px;margin-bottom: 20px;font-size: 15px;font-weight: 400" id="meta_info">
							<span>创作共享平台为您找到相关结果共<em style="color: red;">${beans.resultTotal}</em>条数</span>
							&nbsp&nbsp<span>${beans.msg}</span>
						</div>
						<div class="split-line"></div>
						<div></div>
					...
					<c:if test="${beans.pageTotal<=1}">
					   <a data-page="${beans.pageTotal}"   class="load-more" style="display: none">阅读更多</a>
					</c:if>
					<c:if test="${beans.pageTotal>1}">
						<a data-page="${beans.pageTotal}" class="load-more" id="load-more">阅读更多</a>
					</c:if>
				</div>
...
<script type="text/javascript">
	...
	$(function () {
	    ...
	    function joinData(beans){
	        item=beans.results
	        for(var i=0;i<item.length;i++){
				var li='<li class="have-img">\n' +
					'     <a class="wrap-img" href="/post/'+item[i].id+'" target="_blank">\n' +
					'     <img  class="img-blur-done lazy" style="width: 150px;height: 120px" src="/essay/css/blank.gif" \n' +
					'data-echo="//localhost:80'+item[i].cover+'"/></a>'
					 +
					'     <div class="content">\n' +
					'     <a class="title" target="_blank" href="/post/'+item[i].id+'">'+item[i].title+'</a>\n' +
					'     <p class="abstract">\n' +
					'     '+item[i].previews+'\n' +
					'     </p>\n' +
					'     <div class="meta">\n' +
					'     <a class="collection-tag" target="_blank" href="javascript:;">'+item[i].category+'</a>\n' +
					'     <a target="_blank" href="javascript:;">\n' +
					'     <i class="iconfont ic-list-read"></i> '+item[i].viewCounts+'\n' +
					'     </a>\n' +
					'     <a target="_blank" href="javascript:;">\n' +
					'     <i class="iconfont ic-list-comments"></i> '+item[i].commentCounts+'\n' +
					'     </a>\n' +
					'     <span><i class="iconfont ic-list-like"></i> '+item[i].likes+'</span>\n' +
					' </div>\n' +
					' </div>\n' +
					' </li>'
					 $(".note-list").append(li)
	    ...
	    $(".load-more").click(function () {
			var pagetotal=parseInt($(this).attr('data-page'))
			$.ajax({
				url:'/search/post/'+pagenums,
				type:'GET',
				dataType:'json',
				data:{keyword:"${beans.keyword}",type:"${beans.type}"},
